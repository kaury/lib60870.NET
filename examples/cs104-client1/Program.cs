...
namespace cs104_client1
{
    class MainClass
    {
        ...
        public class Receiver : IFileReceiver
        {
            ...
            public void SegmentReceived(int fileId, int offset, uint size, byte[] data)
            {
                Console.WriteLine("File segment - fileId: {0} offset: {1} size: {2}", fileId, offset, size);
            }
        }
        ...
        public static void Main(string[] args)
        {
            ...
            con.SendReadCommand(1, 0x4001);
            Thread.Sleep(1000);

            con.ReadParameters(1, 0, new int[] { 0x8001 });
            Thread.Sleep(1000);

            con.SendReadClockCommand(1);
            Thread.Sleep(1000);

            con.ReadCurrentSettingArea(1);
